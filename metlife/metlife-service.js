...
class MetLifeService {
  ...
  async extractPatientData(subscriberId, lastName, dateOfBirth, firstName, onLog = console.log) {
    try {
      ...
      // Cliquer sur le champ de recherche - méthode robuste
      try {
        // Essayer d'abord sans .nth(2)
        await this.page.locator('div').filter({ hasText: /^Subscriber ID or Social Security Number$/ }).first().click();
      } catch (e1) {
        try {
          // Fallback vers .nth(2) si nécessaire
          await this.page.locator('div').filter({ hasText: /^Subscriber ID or Social Security Number$/ }).nth(2).click();
        } catch (e2) {
          try {
            // Autre fallback : chercher le champ d'input directement
            await this.page.getByTestId('input-default-input').click();
          } catch (e3) {
            throw new Error(`Could not find search field: ${e1.message}`);
          }
        }
      }
      ...
      // Vérifier si désambiguïsation requise (nouvelle interface)
      const pageText = await this.page.innerText('body').catch(() => '');

      if (pageText.includes('multiple results') || pageText.includes('last name')) {
        onLog('⚠️ Disambiguation detected - new interface with zip code');

        // Nouvelle interface : remplir le nom de famille
        await this.page.waitForSelector('#input-lastname-input', { timeout: 5000 });
        await this.page.locator('#input-lastname-input').fill(lastName);
        onLog(`   Entered last name: ${lastName}`);

        // Remplir le zip code (requis maintenant) - utiliser un zip code par défaut
        const defaultZipCode = '90210'; // Zip code par défaut si pas fourni
        await this.page.locator('#input-zipcode-input').fill(defaultZipCode);
        onLog(`   Entered zip code: ${defaultZipCode} (default)`);

        // Soumettre avec le nouveau bouton "Find"
