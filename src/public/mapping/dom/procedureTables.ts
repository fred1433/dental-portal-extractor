...
function setRadioValue(name: string, value: string | undefined) {
  if (!value) return;
...
function setNoteInput(labelContains: string, value: string | undefined) {
  if (!value) return;
  const inputs = Array.from(document.querySelectorAll('.note-item input[type="text"]')) as HTMLInputElement[];
...
export function applyProcedureHistory(normalized: NormalizedEligibility, raw?: ExtractionResult) {
