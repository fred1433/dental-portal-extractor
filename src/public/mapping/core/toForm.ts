...
export function toFormFieldMap(normalized: NormalizedEligibility, raw: ExtractionResult): FormFieldMap {
  ...
  const maskSSN = (value: string | undefined): string => {
    if (!value) {
      return '';
    }
 ...
 const annualRemaining = coalesce(normalized.maximums.annualRemaining, raw.summary?.annualMaximum?.remaining);
  ...
  if (normalized.procedureLimitations) {
    const dynamicMap = map as Record<string, string>;
  ...
  const getLastDate = (code: string) => (map as any)[`${code}_last_date`] || '';
...
function findWaitingPeriod(waitingPeriods: any[], codes: string[]): any {
