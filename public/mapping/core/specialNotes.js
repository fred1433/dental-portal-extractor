const YES = 'yes';
const NO = 'no';
function getAdditionalBenefit(normalized, header) {
...
export function extractSpecialNotesAnswers(normalized, raw) {
    ...
    if (treatments) {
        ...
        const findTreatment = (code) => list.find(t => (t.treatmentCode ?? t.treatmentcode) === code);
        const toCategory = (treatment) => {
        ...
        if (pd)
            answers.srpCategory = toCategory(pd) ?? 'major';
        if (en)
            answers.endoCategory = toCategory(en) ?? 'major';
        if (os)
...
//# sourceMappingURL=specialNotes.js.map